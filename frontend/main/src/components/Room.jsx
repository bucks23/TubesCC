...
function Room() {
  ...
  useEffect(() => {
    fetch("https://web-production-f02bf.up.railway.app/https://adventurous-motivation-production.up.railway.app/api/rooms")
      .then((res) => res.json())
        ...
        <section className="max-w-7xl mx-auto bg-white rounded-3xl shadow-lg p-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredRooms.map((room) => (
              <article
                key={room.id}
                className="bg-white rounded-2xl shadow-md hover:shadow-xl transition p-0 overflow-hidden flex flex-col"
              >
                <img
